...
char sendir[16];
 if (irrecv.decode(&results)){
   ...
   //    value.toCharArray(sendir,32);
       value = strtoul(sendir,0,16);
       mqtt.publish("temp",sendir);
...
{ int a=8;
  ...
  send_array[0]=2;
    ...
    if((state = digitalRead(PIN_RADAR)) != lastState)
    {
...
void voice(int pin1,int pin2,int snum)
{
 ...
 uint8_t change=RT_soundA - RT_value[snum];
 if (change>1) // If we hear a sound
 {
  RT_value[snum]=RT_soundA;
  Serial.print("sound");
